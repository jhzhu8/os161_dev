 ...
 * 
 * Purpose:
 *  find time difference between two time specs
 * Arguments: none
 * 
 * Returns: time elapsed in sec and nanosec
 */
static void
getinterval(struct timespec before, struct timespec after, time_t* waitSec, uint32_t* waitNs)
{
  uint32_t a_ns = after.tv_nsec;
  time_t a_s = after.tv_sec;
  if(after.tv_nsec < before.tv_nsec) {
    a_ns += 1000000000;
    a_s--;
  }

  *waitSec = a_s - before.tv_sec;
  *waitNs = a_ns - before.tv_nsec;
}
               ...
               unsigned long catnumber)
{
  ...
  //uint32_t before_nsec, after_nsec;
  uint32_t wait_nsec;
  ...
  for(i=0;i<NumLoops;i++) {
    ...
    getinterval(before_time, before_time, &wait_sec, &wait_nsec);
    P(perf_mutex);
          ...
          unsigned long mousenumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
    ...
    /* update wait time statistics */
    //getinterval(before_sec,before_nsec,after_sec,after_nsec,&wait_sec,&wait_nsec);
    getinterval(before_time, after_time, &wait_sec, &wait_nsec);
    P(perf_mutex);
